...
#[test]
    fn get_clip_resolves_a_registered_bundle_by_path() {
        ...
        let _ = audio.register_clip(path.clone(), Arc::from(bytes.as_slice()));

        assert!(audio.get_clip("sfx/confirm.wav").is_some());
        assert!(audio.get_clip("sfx/missing.wav").is_none());
    }
}
