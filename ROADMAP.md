...
- Partially completed: asset manager follow-through — `AssetManifest` / `AssetPack` now support font aliases, and manifest-backed `AssetBundle` handles can retain a loaded pack together with its source manifest path and tracked dependencies. `Engine::load_asset_bundle()` / `reload_asset_bundle()` now cover the common “keep this content set around and rebuild it later” path without forcing games to manually juggle raw manifest paths.
...
1. Asset manager follow-through — manifest-backed bundles are now in place, but content-heavy management UI still wants clearer shared lifetime policy, release semantics, and broader higher-level loading ergonomics than the current mostly-direct asset access path.
