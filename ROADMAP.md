...
Items 1-23 are done. The motorsport management game has the low-level UI primitives it was missing; the remaining gaps are now mostly ergonomics and composition helpers.

Current priority engine issues for this game:

1. UI flow helpers — same-frame rebuild/sync helpers for menus that need to react to responses without duplicating layout code.
2. Data-driven UI templates — card/list/form descriptors or prefab-like builders to reduce hand-authored widget trees.
3. Lightweight UI state organization helpers — keep larger menu state manageable without forcing a retained-mode rewrite.
4. Form/workflow helpers — repeated patterns for validation, confirm/cancel flows, and multi-step dialogs.
5. Deferred rollback follow-up — animation/timer/tween/RNG helpers remain the main systemic gap once gameplay justifies them.
