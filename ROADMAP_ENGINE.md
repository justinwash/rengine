...
- non-fatal scene validation: `validate_editor_scene` walks an authored scene document and returns a `SceneValidationReport` collecting every issue at once (duplicate/dangling/self/cyclic node ids, sprite nodes with no source, missing `kind`, and — when an asset pack/script registry is supplied — missing asset aliases and unregistered scripts), plus `validate_scene_file`/`validate_scene_dir` project-wide sweeps and a `CURRENT_EDITOR_SCENE_VERSION` schema-version check; this is the data contract an editor validation panel and a pre-boot project check both consume
...
1. Stabilize runtime serialization, IDs, and dependency tracking so the editor has trustworthy data contracts.
   - Status: In progress (non-fatal `validate_editor_scene` report, `CURRENT_EDITOR_SCENE_VERSION` schema-version hook, project-wide `validate_scene_file`/`validate_scene_dir` sweeps with cross-scene id-collision checks, stable random node ids, and a clickable editor Validation panel that selects the offending node are landed; remaining work is live/continuous validation and dependency ("used-by") tracking).
