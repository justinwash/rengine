use rengine::{Color, DrawParams, Engine, Frame, Vec2};

use crate::state::{
    Facing, FightGame, FighterData, FighterState, FighterTextures,
};
use crate::{
    FIGHTER_H, FIGHTER_W, GROUND_Y, MAX_HP, SCREEN_H, SCREEN_W,
};
...
pub fn draw(game: &FightGame, engine: &Engine, frame: &mut Frame) {
    ...
    draw_hud(game, frame, screen);


    if game.demo_mode {
        draw_demo_overlay(game, frame, screen);
    }
}
...
fn draw_hud(game: &FightGame, frame: &mut Frame, screen: (u32, u32)) {
    let bar_w = 300.0;
    let bar_h = 24.0;
    let bar_y = 16.0;
    ...
    let p1_bar_x = 20.0;
    frame.hud_rect(p1_bar_x - 2.0, bar_y - 2.0, bar_w + 4.0, bar_h + 4.0, Color::WHITE, screen);
    frame.hud_rect(p1_bar_x, bar_y, bar_w, bar_h, Color::from_rgba8(60, 20, 20, 255), screen);
    let p1_fill = (game.sim.p1.hp.max(0) as f32 / MAX_HP as f32) * bar_w;
    let p1_color = hp_color(game.sim.p1.hp);
    frame.hud_rect(p1_bar_x, bar_y, p1_fill, bar_h, p1_color, screen);


    let p2_bar_x = SCREEN_W as f32 - 20.0 - bar_w;
    frame.hud_rect(p2_bar_x - 2.0, bar_y - 2.0, bar_w + 4.0, bar_h + 4.0, Color::WHITE, screen);
    frame.hud_rect(p2_bar_x, bar_y, bar_w, bar_h, Color::from_rgba8(60, 20, 20, 255), screen);
...
fn draw_demo_overlay(game: &FightGame, frame: &mut Frame, screen: (u32, u32)) {
