...
impl Game for AnimationDemo {
    ...
    fn render(&mut self, engine: &Engine, frame: &mut Frame) {
        ...
        hud.text(-hw + 16.0, hh - 10.0, "SpriteSheet + Animation Demo", 18.0, Color::WHITE, screen, atlas);
        ...
        for (i, (label, anim)) in animations.iter().enumerate() {
            ...
            labels.text(x, y - 6.0, label, 12.0, Color::new(0.7, 0.8, 0.9, 1.0), screen, atlas);

            let frame_text = format!("frame: ({},{})", fc, fr);
            labels.text(x, y - 22.0, &frame_text, 11.0, Color::new(0.5, 0.6, 0.7, 1.0), screen, atlas);
        }

        let sheet_label = frame.canvas(0);
        sheet_label.text(start_x, start_y - y_spacing - 44.0, "Sprite sheet (4x4 grid, 16x16 cells):", 13.0, Color::new(0.6, 0.7, 0.8, 1.0), screen, atlas);
