...
>
> The runtime `Scene2D` loader now includes an adapter for the editor's scene document format, including grouped multi-sprite prefab export, while the separate editor shell continues to mature around a rengine-native shell, multi-document tabs, typed scene properties, real sprite previews, viewport panning, collapsible and resizable panels, native file flow, undo/redo, autosave, and selection ergonomics such as multi-select, box selection, frame-selection, grid-snapped dragging, selection-center guides, translate/rotate/scale gizmos with W/E/R mode switching, per-node rotation in the scene file, duplicate, reparent, reorder, and selection-history shortcuts. The editor now also has a first-pass track-authoring layer with Polygon and Path node kinds, trigger-shape point/tag metadata editing, and script-args metadata fields; near-term work is prioritized around deeper alignment/distribution and geometry-editing workflows, prefab reuse, play-in-editor, UI documents, and data-document tooling so content-heavy 2D games can move out of hand-authored Rust.
