...
>
> The runtime `Scene2D` loader now includes an adapter for the editor's scene document format, including grouped multi-sprite prefab export, while the separate editor shell continues to mature around a rengine-native shell, multi-document tabs, typed scene properties, real sprite previews, viewport panning, collapsible and resizable panels, and native file flow. Near-term editor work is prioritized around undo/redo, richer spatial authoring, gameplay marker and path tools, prefab reuse, play-in-editor, UI documents, and data-document tooling so content-heavy 2D games can move out of hand-authored Rust.
