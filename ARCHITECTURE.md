...
>
> The runtime `Scene2D` loader now includes an adapter for the editor's scene document format, including grouped multi-sprite prefab export, typed metadata helpers, script binding extraction, and a scene script host with targeted event routing helpers. Meanwhile, the separate editor shell continues to mature around a rengine-native shell, multi-document tabs, typed scene properties, real sprite previews, viewport panning, collapsible and resizable panels, native file flow, undo/redo, autosave, and selection ergonomics such as multi-select, box selection, frame-selection, grid-snapped dragging, selection-center guides, a first axis-constrained translate gizmo, duplicate, reparent, reorder, and selection-history shortcuts. The editor is also moving toward conventional desktop usability with explicit App/File/View/Theme top-bar menus and runtime theme switching. Near-term editor work is now prioritized around rotate and scale gizmo follow-up, gameplay marker and path tools, prefab reuse, play-in-editor, UI documents, and data-document tooling so content-heavy 2D games can move out of hand-authored Rust.
