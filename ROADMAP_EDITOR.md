...
- Script attachment + typed-param inspector: a project `scripts.manifest.json` (path + display name + typed param schema) drives an inspector **script picker** (replacing the free-text field; a check marks the attached script, plus a Clear button). Attaching seeds the script's param defaults; each manifest param renders a typed widget (text for String/Number/Color, checkbox for Bool) that commits to `param_<name>` node properties. Params travel on `SceneScriptBinding2D` (typed `param`/`param_f32`/`param_bool`/`param_color`), so a single registered script is configured per-instance instead of needing one `script_path` per behavior. Scene + project validation now build a registry from the manifest (`SceneScriptRegistry2D::from_known_paths`) so unknown `script_path`s surface in the Validation panel. An **Open Script in Editor** button opens the (compiled-Rust) source in the OS editor. Proven in Formula R: the title menu collapsed four `title_*` scripts into one param-driven `menu_action.rs`, and pit-wall command seats now issue an editor-authored `param_command`. **Enum param kind landed:** `ScriptParamKind::Enum` + a `ScriptParamDef.options` list; the inspector renders a **cycle-selector** (a button that advances through the declared options on click) so only valid values can be authored, with `is_valid_value`/`next_option` helpers on the engine side and the `feature-scene-scripts` sample manifest converted to an enum `action` (`spawn`/`clear`) as the proof point. Next: `asset-ref` param kind (an asset picker), and threading enum `options` into scene validation so an off-list `param_<name>` value is flagged (the cycle-selector prevents it at authoring time; validation would catch hand-edited JSON).
